...
def curve(dom) :
    ...
    if jump==1:
        ...
        Jcheck = np.logical_and(Jcheck0[:]==0,Jcheck1[:,0])
        Target[np.logical_not(Jcheck)]=[[np.nan,np.nan]]
        Target=Target.T
        t=np.linspace(0,dom,Jcheck.sum(),endpoint=False)
        print("Jump check")
        print(Jcheck.sum())
...
if __name__=='__main__' :
    ...
    TagP, Domain=curve(domain) #주어진 curve 모양으로

    print("making TagP fin")
    ...
    if nettype==0 :
        ...
        A[0,1:N]=1
        A[0:int((N+1)/2)-1,int((N+1)/2)-1]=1
        A[int((N+1)/2)-1,int((N+1)/2):N]=1
        A[:N-1,N-1]=1
    ...
    for i in range(N):
        for j in range(i+1,N):
            if A[i,j] == 1:
                twodots = np.array([Z[i],Z[j]]).T
                # plt.plot(twodots[0],twodots[1],c=linec, alpha=0.2, linewidth=0.4)
                # plt.plot(twodots[0],twodots[1],c=linec, alpha=0.15, linewidth=0.3)
                # plt.plot(twodots[0],twodots[1],c=linec, alpha=0.09, linewidth=0.35)
                # plt.plot(twodots[0],twodots[1],c=linec, alpha=0.05, linewidth=0.4)

                # plt.plot(twodots[0],twodots[1],c=linec, alpha=0.01, linewidth=0.35)

                # plt.plot(twodots[0],twodots[1],c=linec, alpha=0.6, linewidth=0.3)
                # plt.plot(twodots[0],twodots[1],c=linec, alpha=0.2, linewidth=0.1)
                # plt.plot(twodots[0],twodots[1],c=linec, alpha=0.15, linewidth=0.6)
        
                plt.plot(twodots[0],twodots[1],c=linec, alpha=0.5, linewidth=0.5)
             
                # plt.plot(twodots[0],twodots[1],c=linec, alpha=0.5, linewidth=0.9)
                # plt.plot(twodots[0],twodots[1],c=linec, alpha=0.4, linewidth=0.7)
                # plt.plot(twodots[0],twodots[1],c=linec, alpha=0.3, linewidth=0.5)

                # plt.plot(twodots[0],twodots[1],c=linec, alpha=1, linewidth=0.5)


        if i%10==0:
            print(i)

    print(r'$|\calE|=%f$' %A.sum())
    print(mpl.rcParams['lines.markersize'])

    if dotc!=False:
        scat=plt.scatter(TagP[0],TagP[1],s=0.6*1000/len(Domain)*(fsize/5)**2, c=dotc,cmap=dotcmap,zorder=N*N+1,edgecolors='k',linewidths=0.5)

    print("making graph fin")

    pprr=1    
    if pprr!='0' :
        plt.savefig('net_%s_type%d_N%d.pdf' %(curvename,nettype,N),dpi=3000,bbox_inches='tight',pad_inches=0.02)
    print("DONE")

#plot의 여백 지우기1 https://gldmg.tistory.com/104
#plot의 여백 지우기2 https://www.delftstack.com/ko/howto/matplotlib/how-to-turn-off-the-axes-for-subplots-in-matplotlib/#get_xaxis.set_visible%25EB%25B0%258Fget_yaxis.set_visible
