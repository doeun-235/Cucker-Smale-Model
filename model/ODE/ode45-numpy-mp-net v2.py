...
def makeplot():
    ...
    if graphcut==0 :
        ...
        cx.plot(range(0,T),phE+Vnrm,linewidth=0.5,c='indianred',label=r'$E_t$')
        cx.plot(range(0,T),E0-phE, alpha=0.8,linewidth=0.5,c='forestgreen',label=r'$E_0-E_\phi(t)$')
        ...
        cx_up.plot(range(0,T),E0-phE, alpha=0.8,linewidth=0.5,c='forestgreen',label=r'$E_0-E_\phi(t)$')

        cx_down.plot(range(0,T),[E0 for tt in range(T)],linewidth=0.5,c='gold',label=r'$E_0$')
        cx_down.plot(range(0,T),Vnrm,linewidth=0.7, c='teal', label=r'$\|\|v_t\|\|^2$')
        cx_down.plot(range(0,T),E0-phE, alpha=0.8,linewidth=0.5,c='forestgreen',label=r'$E_0-E_\phi(t)$')
...
def _update_plot (i,fig,scat,qax,ax,reddot,reddot2) : #making animationed plot
    ...
    ax.set_xlabel(r'$\Delta t=%.5f, time : %d$' %(h,i)+'(frame_len)=%.3f\n dBt=%.3f \n' %(xylen,dB[i]),fontsize=8)
...
if __name__ == '__main__':
    ...
    K=0.5
    M=5
    L=0 # since it is not SDE
    # L=0.00001 #L=sigma
    T=500 #T : number of steps for solving the DE
    ...
    for t in range(1,T):
        ...
        K_1=np.array([Vnow,csmpf(Pnow,Vnow)])

        K_2=np.array([Vnow+K_1[1]*h/2,csmpf(Pnow+K_1[0]*h/2,Vnow+K_1[1]*h/2)])

        K_3=np.array([Vnow+K_2[1]*h/2,csmpf(Pnow+K_2[0]*h/2,Vnow+K_2[1]*h/2)])

        K_4=np.array([Vnow+K_3[1]*h,csmpf(Pnow+K_3[0]*h,Vnow+K_3[1]*h)])
        
        Pnext=np.copy(Pnow)
        Vnext=np.copy(Vnow)

        Pnext+=(K_1[0]+2*K_2[0]+2*K_3[0]+K_4[0])*h/6
        Vnext+=(K_1[1]+2*K_2[1]+2*K_3[1]+K_4[1])*h/6
    ...
    if save!='0' :
        ...
        print("Saving...")
        ani.save(savename,dpi=300,fps=60) #조금 더 느린 애니메이션으로 저장하는 것도 나쁘지 않을 듯
