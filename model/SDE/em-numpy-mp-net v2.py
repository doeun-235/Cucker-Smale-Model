...
if __name__ == '__main__':
    ...
    #Xsnrm(t) = sum i |x_i*|^2
    Xsnrm=np.zeros(T)
    Xsnrm[0]=np.sum(np.power(Pinit[:,0]-Z[:,0],2)+np.power(Pinit[:,1]-Z[:,1],2))
    ...
    cx.set_title("||v(t)||^2")
    # cx.set_yscale('log')
    cx.plot(range(0,T),Vnrm)
    reddot, =cx.plot([0],[Vnrm[0]],'r.') #왠지 모르겠는데 reddot뒤의 ,을 빼면 animation 부분에서 에러남. 

    dx.set_title("sum |x_i*(t)|^2")
    # dx.set_yscale('log')
    
    dx.plot(range(0,T),Xsnrm)
    reddot2, =dx.plot([0],[Xsnrm[0]],'r.') #왠지 모르겠는데 reddot뒤의 ,을 빼면 animation 부분에서 에러남. 
