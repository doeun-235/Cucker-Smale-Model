...
def makeplot():
    ...
    bx=fig.add_subplot(spec[0:3,1])

    dx=fig.add_subplot(spec[3:,1])
    ...
    for i in range(N):
        ax.plot(range(0,T),Vcum[0,:,i,0],linewidth=0.5)
    ...
    for i in range(Trial) :
        bx.plot(range(0,T),Vcum[i,:,0,0],linewidth=0.3, alpha=0.6)

    ## cx : graph of E(Vnrm)
    EVnrm=np.mean(Vnrmcum[:],axis=0)
    EphE=np.mean(phEcum[:],axis=0)
    EXbnrm=np.mean(Xbnrmcum[:],axis=0)
    VVnrmcum=np.sort(Vnrmcum[:],axis=0)

    E0=EVnrm[0]+EphE[0]
    SupVnrm=max(np.max(VVnrmcum[int(Trial*0.9),:]), np.max(EVnrm))
    
    if E0>= SupVnrm:
        cx_up=fig.add_subplot(spec[3,0])
        cx_down=fig.add_subplot(spec[4:,0])
...
def savedata(savemode) :
    if savemode==1 :
        ...
        selP_df.to_csv('selP_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
        selV_df.to_csv('selV_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
        # selnV_df.to_csv('selnV_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
        selVnrm_df.to_csv('selVnrm_k%.2fm%.2fs%.2fnet%d%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
        selXbnrm_df.to_csv('selXbnrm_k%.2fm%.2fs%.2fnet%d%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
        seldB_df.to_csv('seldB_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
        selcount_df.to_csv('selcount_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
        ...
        P_df.to_csv('P_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
        V_df.to_csv('V_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
        # nV_df.to_csv('nV_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
        Vnrm_df.to_csv('Vnrm_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
        Xbnrm_df.to_csv('Xbnrm_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
    ...
    if savemode!=0 :
        ...
        dB_df.to_csv('dB_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
        Pinit_df.to_csv('Pinit_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
        Vinit_df.to_csv('Vinit_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
        Z_df.to_csv('Z_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
        A_df.to_csv('A_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%d_cut%0.2fstd.csv' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,cut),index=False,sep="\t")
...
if __name__ == '__main__':
    ...
    curvetype=1 
    nettype=[1,4,0]
    PVinitset=0 #whether making or loading the initial data of P,V
    ...
    Pcum=np.zeros((Trial,T,N,2))
    Vcum=np.zeros((Trial,T,N,2))
    nVcum=np.zeros((Trial,T,N,2))

    Vnrmcum=np.zeros((Trial,T))
    phEcum=np.zeros((Trial,T))

    Xbnrmcum=np.zeros((Trial,T))
    dBcum=np.zeros((Trial,T))
    ...
    for trial in range (0,Trial):
        ...
        Pcum[trial,0]=Pinit
        Vcum[trial,0]=Vinit
        Vnrmcum[trial,0]=Vnrmi
        Xbnrmcum[trial,0]=Xbnrmi
        phEcum[trial,0]=phEi
        ...
        for t in range(1,T):
            ...
            Pnow=np.copy(Pcum[trial,t-1])
            Vnow=np.copy(Vcum[trial,t-1])
            ...
            ## appending P,V, dB
            Pcum[trial,t]=np.copy(Pnext)
            Vcum[trial,t]=np.copy(Vnext)
            dBcum[trial,t]=dBt

            ## calculating and appending Vnrm, Xbnrm, phE
            s=np.power(Vnext[:,0],2)+np.power(Vnext[:,1],2)

            Vnrmcum[trial,t]=np.sum(s)
            Xbnrmcum[trial,t]=np.sum(np.power(Pnext[:,0]-Z[:,0],2)+np.power(Pnext[:,1]-Z[:,1],2))

            for i in range(N):
                J = A_ph[i,:]==1
                ssq=(np.power(Pnext[i,0]-Z[i,0]-Pnext[:,0]+Z[:,0],2)+np.power(Pnext[i,1]-Z[i,1]-Pnext[:,1]+Z[:,1],2))[J]

                phEcum[trial,t]+=np.sum(phiEest(ssq,beta,phLB))

            phEcum[trial,t]*=M/2

        ##appending to cumulative data array
        # Pcum[trial,:,:]=np.copy(np.array(P))
        # Vcum[trial]=np.copy(np.array(V))
        # Vnrmcum[trial]=np.copy(np.array(Vnrm))
        # Xbnrmcum[trial]=np.copy(np.array(Xbnrm))
        # dBcum[trial]=np.copy(np.array(dB))
        # phEcum[trial]=np.copy(np.array(phE))
    ...
    medVnrm=np.median(Vnrmcum[:],axis=0)
    stdVnrm=np.std(Vnrmcum[:],axis=0)
    ...
    if plotsave!='0' :
        plt.savefig('graph_k%.2fm%.2fs%.2fnet%d.%d.%dN%dpsLB%.2fphLB%.2ftrial%dh%.5f.png' %(K,M,L,nettype[0],nettype[1],nettype[2],N,psLB,phLB,Trial,h),dpi=200)
    else :
        plt.show()
